<template>
  <div
    class="q-pb-0"
    :class="isLoadingNextQuestion ? 'disabled' : ''"
  >
    <div class="">
    ...
    <q-btn
      label="I`am answered"
      class="q-ma-sm"
      @click="stopRecognition(), isChoseEmotion = true"
      :color="answerText.length < 5 ? 'primary' : 'positive'"
      icon="done"
    ...
    <q-btn
      label="Skip question"
      class="q-ma-sm"
      @click="isSkipQuestionDialogShow = true"
      color="negative"
      icon="close"
    ...
    <q-dialog v-model="isSkipQuestionDialogShow">
      <q-card>
        ...
        <q-card-section>
          <q-btn
            v-for="skipReason in SKIP_REASONS"
            color="secondary"
            :key="skipReason.EN"
            :label="`${skipReason.emoji} ${skipReason[lang]}`"
            @click="skipReasonText = skipReason[lang]"
            class="q-ma-sm"
          />
          <div>Other reason:</div>
          <q-input
            v-model="skipReasonText"
            label="Input reason"
            filled
            type="textarea"
          />
    ...
    >
      ...
      <q-btn
        ...
        :key="emotion.EN"
        :label="`${emotion.emoji} ${emotion[lang]}`"
        :style="{ backgroundColor: emotion.color }"
        flat
        @click="onSelectEmotion(emotion.EN)"
        class="q-ma-sm"
      />
    </div>
